...
function generateExcel(rows, entityName) {
  ...
  const safeName = String(entityName ?? 'Unknown').replace(/[^a-zA-Z0-9_-]/g, '_');
...
async function fetchContext() {
  ...
  try {
    ...
    if (currentContext.selectionUnavailable) {
      setStatus('Grid selection could not be read. Try selecting records and re-opening.', 'error');
    }

    if (count > MAX_EXPORT_RECORDS) {
      setStatus(`Too many records selected (max ${MAX_EXPORT_RECORDS}). Narrow your selection.`, 'error');
